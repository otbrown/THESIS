...
\end{itemize}  
 ...
 \hline
 Return & \\ \hline
 \lstinline$effectiveHamiltonian$ & \emph{Two-dimensional complex double array}. Contains the rank-6 tensor formed by the contraction shown in \cref{fig:vs1-3} reshaped in to a matrix, with dimensions equal to the square of the number of elements in the matrix product state tensor for the relevant site -- (\lstinline$HILBY$\( \times \)\lstinline$rowMax$\( \times \)\lstinline$colMax$) \(\times\) (\lstinline$HILBY$\( \times \)\lstinline$rowMax$\( \times \)\lstinline$colMax$). The eigenvector of this matrix with the smallest real eigenvalue can be reshaped to replace the site tensor. \\ \hline
 Input & \\ \hline
 \lstinline$HILBY$ & \\
 \lstinline$rowMax$ & \\
 \lstinline$colMax$ & \\
